 ...
 * flash attention:
 * 在简单版的基础上 实现 Q 的共享内存
 * 对s_sum的计算每个线程内部先算完 再加到s_sum 减少atomicAdd次数
 */
template <typename T>
    ...
    const bool is_causal) 
{
    ...
    float *s_q = s_red + blockDim.x; // size d
    ...
    for (int k = tid; k < totlen; k += blockDim.x) {
        ...
        float dot = 0.0f;
        for (int i = 0; i < d; ++i) {
            dot += to_float(s_q[i]) * to_float(k_ptr[i]);
        }
...
void flashAttention(const std::vector<T> &h_q, const std::vector<T> &h_k,
    ...
    const size_t smem_bytes = sizeof(float) * (static_cast<size_t>(d) + 2 + static_cast<size_t>(blockDim.x) + static_cast<size_t>(d));
