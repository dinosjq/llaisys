...
template <typename T>
void _topk(size_t *out_idx, T *out_val, const T *vals, size_t batch_size, size_t numel, size_t k) {
    for(size_t batch_id = 0; batch_id < batch_size; ++ batch_id){
        size_t *batch_out_idx = out_idx + batch_id * k;
        T *batch_out_val = out_val + batch_id * k;
        const T *batch_vals = vals + batch_id * numel;
        auto cmp = [&](const size_t &i1, const size_t &i2)-> bool {
            if constexpr (std::is_same_v<T, llaisys::bf16_t> || std::is_same_v<T, llaisys::fp16_t>){
                return llaisys::utils::cast<float>(batch_vals[i1]) > llaisys::utils::cast<float> (batch_vals[i2]);
            }
            else {
                return batch_vals[i1] > batch_vals[i2];
            }
        };
        ...
        for(size_t i = 0; i < k; ++ i){
            size_t p = heap.top();
            heap.pop();
            batch_out_idx[k - i - 1] = p;
            batch_out_val[k - i - 1] = batch_vals[p];
        }
    }
}
...
void _topk_launch(std::byte *out_idx, std::byte *out_val, const std::byte *vals, size_t batch_size, size_t numel, size_t k){
    ...
    return _topk<T>(d_out_idx, d_out_val, d_vals, batch_size, numel, k);
}
