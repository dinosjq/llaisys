...
{
    // 检查设备一致性
    CHECK_SAME_DEVICE(attn_val, q, k_cache, v_cache, block_ids);
    // 检查数据类型一致性
    ...
    ASSERT(block_ids->ndim() == 1, "paged_attention: block_ids must be 1D.");
    // 检查是否符合任务描述
    ...
    ASSERT(attn_val_shape[0] == q_shape[0], "paged_attention: seqlen is not satisfied.");
    ...
    ASSERT(block_ids_shape[0] == nblock, "paged_attention: nblock is not satisfied.");
    ...
    const size_t block_num = k_shape[0];
    ...
    case LLAISYS_DEVICE_CPU:
        return cpu::paged_attention(attn_val->data(), q->data(), k_cache->data(), v_cache->data(), block_ids->data(), block_num, token_num, nblock,
                                        attn_val->dtype(), scale, seqlen, nh, dv, d, totlen, nkvh);
#ifdef ENABLE_NVIDIA_API
    case LLAISYS_DEVICE_NVIDIA:
        return nvidia::paged_attention(attn_val->data(), q->data(), k_cache->data(), v_cache->data(), block_ids->data(), block_num, token_num, nblock,
                                            attn_val->dtype(), scale, seqlen, nh, dv, d, totlen, nkvh);
#endif
    default:
        // 不支持的设备类型
