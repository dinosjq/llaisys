...
__device__ __forceinline__ float warp_reduce_v5(float val) {
#pragma unroll
...
static constexpr size_t BLOCK_DIM_V5 = 256;   // 8 warps
static constexpr size_t NW = 8;
static constexpr size_t ELEMS_PER_LANE = 4;    // HD=128 / 32 lanes

// ── v5: v4 flatten pipeline + 向量化访存 ────────────────────────
// 基于 v4 的结构（COALESCE 模板、task 映射、block 指针预计算、flatten tile），
// 寄存器改为 float4 连续布局，smem→reg 用 copy_4d 向量化加载。
template <typename T, size_t HD, size_t HEADS, size_t TILE_K, size_t COALESCE>
__global__ void flash_decoding_v5_kernel(
...
#pragma unroll
    ...
    // ── Q load: copy_4d 向量化 (bf16→float4) ────────────────────
    if (warp_id < HEADS) {
...
#pragma unroll
    ...
    auto cp_tile = [&](size_t stage, size_t g_tile) {
        ...
        for (size_t idx = tid; idx < TILE_K * BLOCKS_PER_ROW; idx += BLOCK_DIM_V5) {
...
#pragma unroll
        for (size_t h = 0; h < HEADS; ++h) {
            ...
            float a = __expf(reg_max[h] - mn), b2 = __expf(mc - mn);
            reg_max[h] = mn; reg_sum[h] = a * reg_sum[h] + b2 * sc;
            float4 aw = *reinterpret_cast<float4 *>(s_acc + (h * NW + 1) * HD + lane_id * ELEMS_PER_LANE);
            reg_acc[h].x = reg_acc[h].x * a + aw.x * b2;
            reg_acc[h].y = reg_acc[h].y * a + aw.y * b2;
            reg_acc[h].z = reg_acc[h].z * a + aw.z * b2;
            reg_acc[h].w = reg_acc[h].w * a + aw.w * b2;
...
__global__ void flash_decoding_v5_reduce(
...
// ── Host launch ────────────────────────────────────────────────────
template <typename T, size_t HD, size_t HEADS, size_t TILE_K, size_t COALESCE>
void launch_v5_kernel(std::byte *attn_val, std::byte *attn_acc, std::byte *attn_sum, std::byte *attn_max,
                      ...
                      float scale, size_t nh, size_t nkvh)
{
    ...
    auto *kernel = flash_decoding_v5_kernel<T, HD, HEADS, TILE_K, COALESCE>;
    ...
    flash_decoding_v5_reduce<T, HD, COALESCE><<<grid1, blockDim>>>(
...
static void flash_decoding_v5_impl(
...
{
#define V5_LAUNCH(H, TK, CO)                                                          \
    launch_v5_kernel<T, HD, H, TK, CO>(attn_val, attn_acc, attn_sum, attn_max,       \
        q, k_cache, v_cache, block_ids, cut_idx, tot_len,                             \
        token_num, batch_size, max_block_num, tot_block_num, tot_task_num, scale, nh, nkvh)

    if (heads == 6 && tile_k == 8  && coalesce == 1) V5_LAUNCH(6, 8, 1);
    else if (heads == 6 && tile_k == 8  && coalesce == 2) V5_LAUNCH(6, 8, 2);
    else if (heads == 6 && tile_k == 8  && coalesce == 3) V5_LAUNCH(6, 8, 3);
    else if (heads == 6 && tile_k == 8  && coalesce == 4) V5_LAUNCH(6, 8, 4);
    else if (heads == 6 && tile_k == 8  && coalesce == 6) V5_LAUNCH(6, 8, 6);
    else if (heads == 6 && tile_k == 8  && coalesce == 8) V5_LAUNCH(6, 8, 8);
    else if (heads == 6 && tile_k == 16 && coalesce == 1) V5_LAUNCH(6, 16, 1);
    else if (heads == 6 && tile_k == 16 && coalesce == 2) V5_LAUNCH(6, 16, 2);
    else if (heads == 6 && tile_k == 16 && coalesce == 3) V5_LAUNCH(6, 16, 3);
    else if (heads == 6 && tile_k == 16 && coalesce == 4) V5_LAUNCH(6, 16, 4);
    else if (heads == 6 && tile_k == 16 && coalesce == 6) V5_LAUNCH(6, 16, 6);
    else if (heads == 6 && tile_k == 16 && coalesce == 8) V5_LAUNCH(6, 16, 8);
    else if (heads == 3 && tile_k == 8  && coalesce == 1) V5_LAUNCH(3, 8, 1);
    else if (heads == 3 && tile_k == 8  && coalesce == 2) V5_LAUNCH(3, 8, 2);
    else if (heads == 3 && tile_k == 8  && coalesce == 3) V5_LAUNCH(3, 8, 3);
    else if (heads == 3 && tile_k == 8  && coalesce == 4) V5_LAUNCH(3, 8, 4);
    else if (heads == 3 && tile_k == 8  && coalesce == 6) V5_LAUNCH(3, 8, 6);
    else if (heads == 3 && tile_k == 8  && coalesce == 8) V5_LAUNCH(3, 8, 8);
    else if (heads == 3 && tile_k == 16 && coalesce == 1) V5_LAUNCH(3, 16, 1);
    else if (heads == 3 && tile_k == 16 && coalesce == 2) V5_LAUNCH(3, 16, 2);
    else if (heads == 3 && tile_k == 16 && coalesce == 3) V5_LAUNCH(3, 16, 3);
    else if (heads == 3 && tile_k == 16 && coalesce == 4) V5_LAUNCH(3, 16, 4);
    else if (heads == 3 && tile_k == 16 && coalesce == 6) V5_LAUNCH(3, 16, 6);
    else if (heads == 3 && tile_k == 16 && coalesce == 8) V5_LAUNCH(3, 16, 8);
    else if (heads == 2 && tile_k == 8  && coalesce == 1) V5_LAUNCH(2, 8, 1);
    else if (heads == 2 && tile_k == 8  && coalesce == 2) V5_LAUNCH(2, 8, 2);
    else if (heads == 2 && tile_k == 8  && coalesce == 3) V5_LAUNCH(2, 8, 3);
    else if (heads == 2 && tile_k == 8  && coalesce == 4) V5_LAUNCH(2, 8, 4);
    else if (heads == 2 && tile_k == 8  && coalesce == 6) V5_LAUNCH(2, 8, 6);
    else if (heads == 2 && tile_k == 8  && coalesce == 8) V5_LAUNCH(2, 8, 8);
    else if (heads == 2 && tile_k == 16 && coalesce == 1) V5_LAUNCH(2, 16, 1);
    else if (heads == 2 && tile_k == 16 && coalesce == 2) V5_LAUNCH(2, 16, 2);
    else if (heads == 2 && tile_k == 16 && coalesce == 3) V5_LAUNCH(2, 16, 3);
    else if (heads == 2 && tile_k == 16 && coalesce == 4) V5_LAUNCH(2, 16, 4);
    else if (heads == 2 && tile_k == 16 && coalesce == 6) V5_LAUNCH(2, 16, 6);
    else if (heads == 2 && tile_k == 16 && coalesce == 8) V5_LAUNCH(2, 16, 8);
    else if (heads == 1 && tile_k == 8  && coalesce == 1) V5_LAUNCH(1, 8, 1);
    else if (heads == 1 && tile_k == 8  && coalesce == 2) V5_LAUNCH(1, 8, 2);
    else if (heads == 1 && tile_k == 8  && coalesce == 3) V5_LAUNCH(1, 8, 3);
    else if (heads == 1 && tile_k == 8  && coalesce == 4) V5_LAUNCH(1, 8, 4);
    else if (heads == 1 && tile_k == 8  && coalesce == 6) V5_LAUNCH(1, 8, 6);
    else if (heads == 1 && tile_k == 8  && coalesce == 8) V5_LAUNCH(1, 8, 8);
    else if (heads == 1 && tile_k == 16 && coalesce == 1) V5_LAUNCH(1, 16, 1);
    else if (heads == 1 && tile_k == 16 && coalesce == 2) V5_LAUNCH(1, 16, 2);
    else if (heads == 1 && tile_k == 16 && coalesce == 3) V5_LAUNCH(1, 16, 3);
    else if (heads == 1 && tile_k == 16 && coalesce == 4) V5_LAUNCH(1, 16, 4);
    else if (heads == 1 && tile_k == 16 && coalesce == 6) V5_LAUNCH(1, 16, 6);
    else if (heads == 1 && tile_k == 16 && coalesce == 8) V5_LAUNCH(1, 16, 8);
    else ASSERT(false, "flash_decoding_v5: unsupported (heads, tile_k, coalesce)");
...
template <typename T>
static void flash_decoding_v5_dispatch(
    std::byte *attn_val, std::byte *attn_acc, std::byte *attn_sum, std::byte *attn_max,
    const std::byte *q, const std::byte *k_cache, const std::byte *v_cache,
    const std::byte *block_ids, const std::byte *cut_idx, const std::byte *tot_len,
    size_t token_num, size_t batch_size, size_t max_block_num,
...
{
    ASSERT(d == dv, "flash_decoding_v5: d must equal dv.");
#define V5_IMPL(HD)                                                                   \
    flash_decoding_v5_impl<T, HD>(attn_val, attn_acc, attn_sum, attn_max,            \
        q, k_cache, v_cache, block_ids, cut_idx, tot_len,                             \
        token_num, batch_size, max_block_num, tot_block_num, tot_task_num,            \
        scale, nh, nkvh, heads, tile_k, coalesce)
    switch (d) {
    case 128: V5_IMPL(128); break;
    case 64:  V5_IMPL(64);  break;
    case 32:  V5_IMPL(32);  break;
    case 16:  V5_IMPL(16);  break;
    case 8:   V5_IMPL(8);   break;
    default:  ASSERT(false, "flash_decoding_v5: unsupported head_dim.");
    }
#undef V5_IMPL
}

// ── C wrapper ───────────────────────────────────────────────────────
extern "C" __export void llaisysFlashDecodingV5(
    ...
    int heads, int tile_k, int coalesce)
{
    ...
    switch (dtype) {
    case 0: flash_decoding_v5_dispatch<float>(ba, bac, bas, bam, bq, bkc, bvc, bb, bc, bt,
        token_num, batch_size, max_block_num, tot_block_num, ttn, scale, nh, dv, d, nkvh, heads, tile_k, coalesce); break;
    case 1: flash_decoding_v5_dispatch<llaisys::fp16_t>(ba, bac, bas, bam, bq, bkc, bvc, bb, bc, bt,
        token_num, batch_size, max_block_num, tot_block_num, ttn, scale, nh, dv, d, nkvh, heads, tile_k, coalesce); break;
    case 2: flash_decoding_v5_dispatch<llaisys::bf16_t>(ba, bac, bas, bam, bq, bkc, bvc, bb, bc, bt,
        token_num, batch_size, max_block_num, tot_block_num, ttn, scale, nh, dv, d, nkvh, heads, tile_k, coalesce); break;
    default: ASSERT(false, "flash_decoding_v5: bad dtype");
    }
    CUDA_CHECK(cudaGetLastError());
}
