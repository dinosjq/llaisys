...
template <typename T>
__global__ void rope_inv_freq_kernel(T *__restrict__ _out, const T *__restrict__ _in,
                                     const int64_t *__restrict__ _pos_ids, const float *__restrict__ _inv_freq,
                                     const size_t _seqlen, const size_t _head, const size_t _d) {
    const int i = blockIdx.y;
    const int h = blockIdx.x;
    const int tid = threadIdx.x;
    ...
    for (int col = (tid << 2); col < hf_d; col += 128) {
        if (col + 3 < hf_d) {
            const float phi_0 = pi * _inv_freq[col | 0];
            const float cos_phi_0 = cosf(phi_0);
            const float sin_phi_0 = sinf(phi_0);

            const float phi_1 = pi * _inv_freq[col | 1];
            const float cos_phi_1 = cosf(phi_1);
            const float sin_phi_1 = sinf(phi_1);

            const float phi_2 = pi * _inv_freq[col | 2];
            const float cos_phi_2 = cosf(phi_2);
            const float sin_phi_2 = sinf(phi_2);

            const float phi_3 = pi * _inv_freq[col | 3];
            const float cos_phi_3 = cosf(phi_3);
            const float sin_phi_3 = sinf(phi_3);
            ...
            for (int j = col; j < hf_d; ++j) {
                const float phi = pi * _inv_freq[j];
                const float cos_phi = cosf(phi);
                const float sin_phi = sinf(phi);
...
template <typename T>
void rope_inv_freq_launch(std::byte *out, const std::byte *in, const std::byte *pos_ids, const std::byte *inv_freq,
                          const size_t &seqlen, const size_t &head, const size_t &d) {
    auto *d_out = reinterpret_cast<T *>(out);
    const auto *d_in = reinterpret_cast<const T *>(in);
    const auto *d_pos_ids = reinterpret_cast<const int64_t *>(pos_ids);
    const auto *d_inv_freq = reinterpret_cast<const float *>(inv_freq);

    dim3 blockDim(32);
    dim3 gridDim(head, seqlen);

    rope_inv_freq_kernel<<<gridDim, blockDim>>>(d_out, d_in, d_pos_ids, d_inv_freq, seqlen, head, d);
...
void rope_inv_freq(std::byte *out, const std::byte *in, const std::byte *pos_ids, const std::byte *inv_freq,
                   llaisysDataType_t type, const size_t &seqlen, const size_t &head, const size_t &d) {
    switch (type) {
    case LLAISYS_DTYPE_F32:
        return rope_inv_freq_launch<float>(out, in, pos_ids, inv_freq, seqlen, head, d);
    case LLAISYS_DTYPE_BF16:
        return rope_inv_freq_launch<llaisys::bf16_t>(out, in, pos_ids, inv_freq, seqlen, head, d);
    case LLAISYS_DTYPE_F16:
        return rope_inv_freq_launch<llaisys::fp16_t>(out, in, pos_ids, inv_freq, seqlen, head, d);
    default:
        EXCEPTION_UNSUPPORTED_DATATYPE(type);
    }
}
