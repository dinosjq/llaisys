...
target("llaisys-core")
    ...
    -- Model 基类（纯头）+ 注册表（不含依赖 ops 的 Layer / 具体模型，避免 core↔ops 环）
    add_files("src/engine/registry/*.cpp")
...
target("llaisys")
    ...
    add_files("src/engine/worker/*.cpp")
    add_files("src/sampler/*.cpp")
    add_files("src/models/qwen2/*.cpp")
    add_files("src/models/qwen2/runtime/*.cpp")
    add_files("src/models/llama/*.cpp")
    add_files("src/models/qwen2/layer/**/*.cpp")
    add_files("src/models/llama/layer/**/*.cpp")
    set_installdir(".")
