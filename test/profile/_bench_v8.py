#!/usr/bin/env python3
"""v8 vs v6 vs FlashInfer benchmark."""
...
script = os.path.join(os.path.dirname(__file__), '_run_v8.py')
...
for (batch, totlen) in configs:
    ...
    a6=(av.data_ptr(),acc.data_ptr(),asum.data_ptr(),amax.data_ptr(),
        qt.data_ptr(),kt.data_ptr(),vt.data_ptr(),tbids.data_ptr(),torch.arange(batch+1,dtype=torch.int64,device=DEV).data_ptr(),tot.data_ptr(),
    ...
    # v8
    a8=(av.data_ptr(),qt.data_ptr(),kt.data_ptr(),vt.data_ptr(),tbids.data_ptr(),tot.data_ptr(),
        TN,batch,mb,1,BV2,s,NH,HD,HD,NKVH)
    for _ in range(3): lib.llaisysFlashDecodingV8(*a8)
    torch.cuda.synchronize()
    torch.cuda.nvtx.range_push(f"V8_B{{batch}}T{{totlen}}")
    lib.llaisysFlashDecodingV8(*a8); torch.cuda.synchronize()
    torch.cuda.nvtx.range_pop()
    # FlashInfer
...
print("Running nsys...")
subprocess.run(["nsys","profile","-t","cuda,nvtx","-o","/tmp/v8b","--force-overwrite=true",
                "python3",script], check=False, timeout=900, capture_output=True, text=True)
subprocess.run(["rm","-f","/tmp/v8b_t*.csv"], check=False)
subprocess.run(["nsys","stats","--report=cuda_gpu_trace","--format=csv","--force-export=true",
                "-o","/tmp/v8b_t","/tmp/v8b.nsys-rep"], check=True, timeout=120, capture_output=True)

kernels = []
with open("/tmp/v8b_t_cuda_gpu_trace.csv") as f:
    r = csv.reader(f); h = next(r); ni = h.index("Name"); di = h.index("Duration (ns)")
    for row in r:
        n = row[ni]; d = int(row[di])
        if 'flash_decoding_v6' in n or 'flash_decoding_v8' in n or 'BatchDecodeWithPagedKVCacheKernel' in n:
            kernels.append((d, n))
print(f"Total: {len(kernels)}")
per = 8 + 4 + 4  # v6 3w+1m x2=8, v8 3w+1m=4, FI 3w+1m=4 = 16
print(f"\n{'b×totlen':>14} {'v6(us)':>8} {'v8(us)':>8} {'FI(us)':>8} {'v8/v6':>7} {'FI/v8':>7}")
for ci,(b,t) in enumerate(configs):
    base=ci*16
    v6_t=kernels[base+6][0]/1e3+kernels[base+7][0]/1e3
    v8_t=kernels[base+10][0]/1e3+kernels[base+11][0]/1e3
    fi_t=kernels[base+15][0]/1e3
    print(f"  {b:2d}×{t:5d}  {v6_t:6.1f}  {v8_t:6.1f}  {fi_t:6.1f}  {v8_t/v6_t:.2f}x  {fi_t/v8_t:.2f}x")
