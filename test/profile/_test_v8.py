...
# V8: 6 void* + 4 size_t + (int,float) + 4 size_t
lib.llaisysFlashDecodingV8.argtypes = [c_void_p]*6 + [c_size_t]*4 + [c_int, c_float] + [c_size_t]*4

for batch,totlen in [(1,256),(1,2048),(4,2048),(16,512),(32,256)]:
    ...
    av3=torch.empty((batch,NH,HD),dtype=torch.bfloat16,device=DEV)
    a3=(av3.data_ptr(),acc.data_ptr(),asum.data_ptr(),amax.data_ptr(),
        qt.data_ptr(),kt.data_ptr(),vt.data_ptr(),tbids.data_ptr(),cut.data_ptr(),tot.data_ptr(),
        TN,batch,mb,tb,1,BV2,s,NH,HD,HD,NKVH,6,8,0)
    lib.llaisysFlashDecodingV3(*a3); torch.cuda.synchronize()
    # v8 (no acc/sum/max, no cut_idx)
    av8=torch.empty((batch,NH,HD),dtype=torch.bfloat16,device=DEV)
    a8=(av8.data_ptr(),qt.data_ptr(),kt.data_ptr(),vt.data_ptr(),
        tbids.data_ptr(),tot.data_ptr(),TN,batch,mb,1,BV2,s,NH,HD,HD,NKVH)
    lib.llaisysFlashDecodingV8(*a8); torch.cuda.synchronize()
    diff=(av3.float()-av8.float()).abs().max().item()
    ok = "OK" if diff<0.5 else f"FAIL {diff:.4f}"
    print(f"  {batch}x{totlen}: v3 vs v8 {ok}")
print("done")
