...
for batch,totlen in [(1,256),(1,2048),(4,2048),(16,512),(32,256)]:
    ...
    for tail in [(6,8,1),(6,8,4),(6,16,1),(3,16,1),(2,16,1)]:
        v9 = run("llaisysFlashDecodingV9", tail)
        diff=(v3.float()-v9.float()).abs().max().item()
        ok = "OK" if diff<0.5 else f"FAIL {diff:.4f}"
        print(f"  {batch}x{totlen} H{tail[0]}T{tail[1]}C{tail[2]}: {ok}")
print("done")
