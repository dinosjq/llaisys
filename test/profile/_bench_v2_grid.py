#!/usr/bin/env python3
"""全网格参数扫描: batch × totlen × (HEADS × TILE_K)，找最优参数映射."""
...
print("=== 最优参数 per (batch, totlen) ===")
print(f"{'batch':>5} {'totlen':>6} {'tokens':>7} {'best(us)':>9} {'heads':>6} {'tile_k':>7}  top3")
print("-" * 70)
for batch in BATCHES:
    for totlen in TOTLENS:
        results = []
        for heads, tk in PARAMS:
            lat = run_v2(batch, totlen, heads, tk)
            results.append((lat, heads, tk))
        results.sort()
        best = results[0]
        top3 = "  ".join(f"H{h}/T{t}={l:.1f}" for l, h, t in results[:3])
        print(f"{batch:5d} {totlen:6d} {batch*totlen:7d} {best[0]:9.1f} {best[1]:6d} {best[2]:7d}  {top3}")

# 汇总: heads/tile_k 分布的多数表决
print("\n=== 参数分布统计 ===")
from collections import Counter
hcount = Counter(); tcount = Counter()
for batch in BATCHES:
    for totlen in TOTLENS:
        results = []
        for heads, tk in PARAMS:
            results.append((run_v2(batch, totlen, heads, tk), heads, tk))
        results.sort()
        _, h, t = results[0]
        hcount[h] += 1; tcount[t] += 1
print(f"  最优 HEADS 分布: {dict(hcount)}")
print(f"  最优 TILE_K 分布: {dict(tcount)}")
