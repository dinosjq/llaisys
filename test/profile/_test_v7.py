...
for batch,totlen in [(1,256),(1,2048),(4,2048),(16,512),(32,256)]:
    ...
    def run(fn, tail):
        ...
        a=(av.data_ptr(),acc.data_ptr(),asum.data_ptr(),amax.data_ptr(),
           qt.data_ptr(),kt.data_ptr(),vt.data_ptr(),tbids.data_ptr(),cut.data_ptr(),tot.data_ptr(),
           TN,batch,mb,tb,1,BV2,s,NH,HD,HD,NKVH)+tail
        getattr(lib,fn)(*a); torch.cuda.synchronize()
        return av
    v3 = run("llaisysFlashDecodingV3", (6,8,0))
    for tail in [(6,8,1,2,0),(6,8,1,2,1),(6,16,1,2,1),(3,16,1,2,1),(2,16,1,2,1)]:
        v7 = run("llaisysFlashDecodingV7", tail)
        diff=(v3.float()-v7.float()).abs().max().item()
        ok = "OK" if diff<0.5 else f"FAIL {diff:.4f}"
        print(f"  {batch}x{totlen} H{tail[0]}T{tail[1]}C{tail[2]}S{tail[3]}U{tail[4]}: {ok}")
print("done")
