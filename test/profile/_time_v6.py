...
for batch, totlen in [(1,256),(16,2048)]:
    ...
    for h,tk,co in [(6,8,1),(6,8,4),(3,16,4),(2,16,1)]:
        ...
        # warmup
        for _ in range(20): lib.llaisysFlashDecodingV6(*args)
        torch.cuda.synchronize()
        best=1e9
        for _ in range(50):
            torch.cuda.synchronize(); t0=time.perf_counter()
            lib.llaisysFlashDecodingV6(*args)
            torch.cuda.synchronize()
            best=min(best,(time.perf_counter()-t0)*1e6)
        print(f"  {batch}x{totlen} H{h}T{tk}C{co}: {best:.1f}us")
