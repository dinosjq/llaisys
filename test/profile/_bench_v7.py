...
lib.llaisysFlashDecodingV7.argtypes = [c_void_p]*10 + [c_size_t]*4 + [c_size_t, c_int, c_float] + [c_size_t]*4 + [c_int]*5
...
script = os.path.join(os.path.dirname(__file__), '_run_v7.py')
...
print("Running nsys...")
subprocess.run(["nsys","profile","-t","cuda,nvtx","-o","/tmp/v7b","--force-overwrite=true",
                "python3",script], check=False, timeout=900, capture_output=True, text=True)
subprocess.run(["rm","-f","/tmp/v7b_t*.csv"], check=False)
subprocess.run(["nsys","stats","--report=cuda_gpu_trace","--format=csv","--force-export=true",
                "-o","/tmp/v7b_t","/tmp/v7b.nsys-rep"], check=True, timeout=120, capture_output=True)

kernels = []
with open("/tmp/v7b_t_cuda_gpu_trace.csv") as f:
    r = csv.reader(f); h = next(r); ni = h.index("Name"); di = h.index("Duration (ns)")
    for row in r:
        n = row[ni]; d = int(row[di])
        if 'flash_decoding_v6' in n or 'flash_decoding_v7' in n or 'BatchDecodeWithPagedKVCacheKernel' in n:
            kernels.append((d, n))
print(f"Total: {len(kernels)}")
# Per config: v6 (3w+1m)x2=8, v7 8, FI 4 = 20 kernels
per = 20
print(f"\n{'b×totlen':>14} {'v6(us)':>8} {'v7(us)':>8} {'FI(us)':>8} {'v7/v6':>7} {'FI/v7':>7}")
for ci,(b,t) in enumerate(configs):
    base=ci*per
    v6_t=kernels[base+6][0]/1e3+kernels[base+7][0]/1e3
    v7_t=kernels[base+14][0]/1e3+kernels[base+15][0]/1e3
    fi_t=kernels[base+19][0]/1e3
    print(f"  {b:2d}×{t:5d}  {v6_t:6.1f}  {v7_t:6.1f}  {fi_t:6.1f}  {v7_t/v6_t:.2f}x  {fi_t/v7_t:.2f}x")
