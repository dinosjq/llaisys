#!/usr/bin/env python3
"""LLAISYS v4 vs FlashInfer: BATCH_MAX_TOKEN_NUM=8192 configs, nsys timing."""
...
NH, NKVH, HD, TN = 8, 2, 128, 64  # group_size=4 for FlashInfer compat
...
# Best (H,TK,CO) per config from v4 detail scan (NH=8 config)
best_params = {
    (1,256): (2,16,1), (1,512): (2,16,1), (1,1024): (2,16,1),
    (1,2048): (6,8,1), (1,4096): (6,8,1),
    (4,2048): (6,8,1), (8,1024): (6,8,1),
    (16,512): (6,8,1), (32,256): (6,8,1),
}
...
for batch, totlen in configs:
    ...
    for _ in range(3): wrapper.run(q_fi, (k_fi, v_fi))
    ...
    # === LLAISYS v4 (best params) ===
    h, tk, co = best_params[(batch, totlen)]
    ...
    torch.cuda.nvtx.range_push(f"LL_B{batch}T{totlen}_H{h}K{tk}C{co}")
    lib.llaisysFlashDecodingV4(*args); torch.cuda.synchronize()
    torch.cuda.nvtx.range_pop()
